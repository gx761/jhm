...
function cmsms_woocommerce_cart_dropdown() {
	...
	$output .= '<div class="cmsms_dynamic_cart">' .  
		...
		'<div class="widget_shopping_cart_content_wrap"><div class="widget_shopping_cart_content"></div></div>' . 
	'</div>';
...
function cmsms_woocommerce_add_to_cart_button() {
	...
	if ($product->is_purchasable() && $product->product_type == 'simple' && $product->is_in_stock()) {
		echo '<a href="' . esc_url($product->add_to_cart_url()) . '" data-product_id="' . esc_attr($product->id) . '" data-product_sku="' . esc_attr($product->get_sku()) . '" class="add_to_cart_button cmsms_add_to_cart_button product_type_simple">' . __('Add to Cart', 'cmsmasters') . '</a>';
	}
...
function woocommerce_header_add_to_cart_fragment( $fragments ) {
	...
	<?php $fragments['div.cmsms_wrap_basket'] = ob_get_clean();
