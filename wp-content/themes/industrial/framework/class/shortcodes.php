	...
	if ($button != '') {
        $out .= '<div class="colored_button">' . 
			'<a href="' . $buttonlink . '" title="' . $buttontext . '"';
				...
				$out .= '>' . 
				'<span class="icon_banner icon_' . $buttonicon . '"></span>' . 
...
function cmsmasters_colored_block($atts, $content = null) {
    ...
    $out = '<div class="colored_banner" style="background-color:' . $bgcolor . ';">' . do_shortcode($content);
...
function cmsmasters_person_block($atts, $content = null) {
	...
	$out = '<div class="cmsms_our_team">';
	
	
	if ($image != '') {
		$out .= '<figure>' . 
			wp_get_attachment_image($image, 'slider-thumb', false, array( 
				'class' => 'fullwidth' 
			)) . 
		'</figure>';
	}
	...
	$out .= '<div class="cmsms_team_rollover">' . 
		...
		if ($links_texts != '' && $links_links != '') {
			...
			for ($i = 0; $i < count($links_links_array); $i++) {
				$out .= '<a href="' . $links_links_array[$i] . '" title="' . $links_texts_array[$i] . '" target="_blank" class="person_social">' . (($links_imgs_array[$i] != '') ? '<img src="' . $links_imgs_array[$i] . '" alt="' . $links_texts_array[$i] . '" />' : $links_texts_array[$i]) . '</a>';
			}
...
function cmsmasters_tabs($atts, $content = null) {
	...
	for ($i = 0; sizeof($shortcode_array) > $i; $i++) {
		...
		foreach($pairs as $pair) {
			$atr = array_map("trim_quotes_tab", preg_split("/\s*=\s*/", $pair));
...
function cmsmasters_pricing_table($atts, $content = null) {
	...
	$out = '<div class="cmsms_pricing_table">' . 
		...
		'<div class="pricing_footer">' . 
			'<a class="pricing_button buy" href="' . $buttonlink . '">' . $buttontext . '</a>' . 
		'</div>' . 
	'</div>';
...
function cmsmasters_stats_bar($atts, $content = null) {
	...
	return '<div class="percent_item">' . 
		'<div class="percent_item_colored_wrap" style="width:' . $value . '%;">' . 
			...
			'<span class="percent_item_text">' . $title . ' ' . $value . '%</span> ' . 
		'</div>' . 
	'</div>';
}
...
function posttype_shortcode($atts, $content = null) {
	...
	if ($post_slide == 'true') {
		$out = '<section id="portfolio_shortcode_' . $unid . '" class="post_type_shortcode'; 
			...
			$out .= '"><div class="post_type_shortcode_inner"><h2>' . $post_title . '</h2>' . 
    ...
    if ($posttype_query->have_posts()) :
        while ($posttype_query->have_posts()) : $posttype_query->the_post();
			...
			if ($post_type != 'testimonial') {
				...
				$out .= '<div class="project_rollover">' . 
				...
				if ($show_images == 'true') {
					if (has_post_thumbnail() != '') {
						$out .= get_the_post_thumbnail(get_the_ID(), 'thumbnail', array( 
							'alt' => cmsms_title(get_the_ID(), false), 
							'title' => cmsms_title(get_the_ID(), false), 
