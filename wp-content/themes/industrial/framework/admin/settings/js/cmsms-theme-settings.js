...
(function ($) { 
	...
	$('.icon_management').on('click', '.icon_edit', function () { 
		...
		$('#new_icon_title').val(edit_icon_val[3]);
		
		$('.icon_upload_link').find('input[type="checkbox"]').prop('checked', ((edit_icon_val[4] == 'true') ? true : false));
	...
	$('#edit_icon').click(function () { 
		...
		if ($('#new_icon_name').val() !== '') {
			$('input#' + edit_icon_data_id).val($('#new_icon_name').val() + '|' + $('#new_icon_color').val() + '|' + $('#new_icon_link').val() + '|' + $('#new_icon_title').val() + '|' + (($('#new_icon_target').is(':checked')) ? 'true' : 'false'));
