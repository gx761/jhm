...
function register_js_scripts() {
	if (!is_admin()) {
		...
		wp_register_script('easing', get_template_directory_uri() . '/js/jquery.easing.min.js', array('jquery'), '1.3.0', true);
...
function cmsms_theme_google_fonts_generate() {
	...
	foreach (cmsms_google_fonts_list() as $key => $value) {
		if ( 
			...
			(isset($cmsms_option[CMSMS_SHORTNAME . '_footer_font_google_font']) && $key == $cmsms_option[CMSMS_SHORTNAME . '_footer_font_google_font'] && $key != '') 
		) {
...
if (function_exists('add_theme_support')) {
    ...
    set_post_thumbnail_size(820, 365, true);
}

if (function_exists('add_image_size')) {
	add_image_size('project-thumb', 440, 500, true);
	add_image_size('project-thumb-half', 580, 660, true);
	add_image_size('project-thumb-full', 1160, 1320, true);
	add_image_size('project-slider-thumb-full', 1160, 9999);
	add_image_size('project-slider-thumb', 820, 9999);
	add_image_size('slider-thumb', 820, 9999);
	add_image_size('full-thumb', 1085, 435, true);
	add_image_size('full-slider-thumb', 1085, 9999);
	add_image_size('latest-thumb', 675, 610, true);
}
...
/* Bottom Twitter Widget Function */
function cmsms_get_tweets($username, $count) {
	...
	$connection = new TwitterOAuth( 
		'HhcPg8RPPT2xiMPFSPNtFg', 
		'xftjXm8KSo80jNlD8SDLNxq9et6YW5bVzrLfkc7yE', 
		'298558518-SJQ7PPgoIFBw9UBARwVKekYkJlfmTClLS0g9ldm6', 
		'ndSjXVySnjK7st5QBo4yuRyK8k3yOqKsS1p6ZMjhM' 
	);
...
/* Parse Tweets Function */
function parse_feed($feed) {
	$pattern = "#<content(?:\s+[^>]+)?>(.*?)<\/content>#s";
	
	preg_match_all($pattern, $feed, $matches);
	
	
	$tweets = '<ul class="jta-tweet-list responsiveContentSlider">';
	
	foreach ($matches[1] as $match) {
		$tweets .= '<li class="jta-tweet-list-item">' . html_entity_decode($match) . '</li>';
	}
...
function cmsms_header_styles() {
	...
	echo '#middle.cmsms_middle_margin { ' . 
			'margin-top : ' . $cmsms_option[CMSMS_SHORTNAME . '_header_height'] . 'px; ' . 
		'} ' . 
		'#header > .header_inner { ' . 
			'height : ' . $cmsms_option[CMSMS_SHORTNAME . '_header_height'] . 'px; ' . 
		'} ' . 
		'#header.fixed_header + * { ' . 
			'margin-top : ' . $fixed_header_margin_top . 'px; ' . 
		'} ' . 
		'#header > .header_inner > a.logo { ' . 
			'top : ' . $cmsms_option[CMSMS_SHORTNAME . '_logo_top'] . 'px; ' . 
			'left : ' . $cmsms_option[CMSMS_SHORTNAME . '_logo_left'] . 'px; ' . 
		'} ' . 
		'.header_inner:after { ' . 
			'width : ' . (($cmsms_option[CMSMS_SHORTNAME . '_logo_width'] < 255) ? 255 : $cmsms_option[CMSMS_SHORTNAME . '_logo_width']) . 'px; ' . 
		'} ' . 
		...
		'} ' . 
		'#header nav { ' . 
			'top : ' . $cmsms_option[CMSMS_SHORTNAME . '_header_nav_top'] . 'px; ' . 
			'left : ' . (($cmsms_option[CMSMS_SHORTNAME . '_logo_width'] < 255) ? 255 : $cmsms_option[CMSMS_SHORTNAME . '_logo_width']) . 'px; ' . 
		'} ' . 
		'#header .social_icons { ' . 
			'top : ' . $cmsms_option[CMSMS_SHORTNAME . '_header_social_top'] . 'px; ' . 
			'left : ' . (($cmsms_option[CMSMS_SHORTNAME . '_logo_width'] < 255) ? 255 : $cmsms_option[CMSMS_SHORTNAME . '_logo_width']) . 'px; ' . 
		'} ' . 
		'#header .search_line { ' . 
			'top : ' . $cmsms_option[CMSMS_SHORTNAME . '_header_height']/2 . 'px; ' . 
			'left : ' . (($cmsms_option[CMSMS_SHORTNAME . '_logo_width'] < 255) ? 255 : $cmsms_option[CMSMS_SHORTNAME . '_logo_width']) . 'px; ' . 
		'} ' . 
		'.header_html { ' . 
			'top : ' . $cmsms_option[CMSMS_SHORTNAME . '_header_custom_html_top'] . 'px; ' . 
			'right : ' . $cmsms_option[CMSMS_SHORTNAME . '_header_custom_html_right'] . 'px; ' . 
		'} ' . 
		'#header.fixed_header > .header_inner { ' . 
			'height : ' . $fixed_header_height . 'px; ' . 
		'} ' . 
		'#header.fixed_header > .header_inner > a.logo { ' . 
			'top : ' . ($cmsms_option[CMSMS_SHORTNAME . '_logo_top'] - $margin_difference) . 'px; ' . 
		'} ' . 
		'#header.fixed_header nav { ' . 
			'top : ' . ($cmsms_option[CMSMS_SHORTNAME . '_header_nav_top'] - $margin_difference) . 'px; ' . 
		'} ' . 
		'#header.fixed_header .social_icons { ' . 
			'top : ' . ($cmsms_option[CMSMS_SHORTNAME . '_header_social_top'] - $margin_difference) . 'px; ' . 
		'} ' . 
		'#header.fixed_header .search_line { ' . 
			'top : ' . $fixed_header_height/2 . 'px; ' . 
		'} ' . 
...
add_action('wp_head', 'cmsms_header_styles');
