...
function cmsms_heading_nolink($cmsms_id, $type = 'post', $show = true, $tag = 'h1') { 
	...
	if ($type == 'post') { // Post type - post
		if ( 
			$cmsms_option[CMSMS_SHORTNAME . '_blog_post_title'] && 
			$cmsms_heading != 'parallax' 
		) {
			if ($show) {
		...
		if ( 
			$cmsms_option[CMSMS_SHORTNAME . '_portfolio_project_title'] && 
			$cmsms_heading != 'parallax' 
		) { 
			if ($show) {
				echo '<' . $tag . ' class="entry-title">' . 
...
function cmsms_heading($cmsms_id, $type = 'post', $show = true, $tag = 'h3') { 
	...
	if ($content_type == 'post') { // Post type - post
		if ($template_type == 'post') { // Template type - post
			if ($cmsms_option[CMSMS_SHORTNAME . '_blog_post_cat'] && get_the_category()) {
				if ($cmsms_option[CMSMS_SHORTNAME . '_blog_post_comment'] || $cmsms_option[CMSMS_SHORTNAME . '_blog_page_date']) {
			...
			if ($cmsms_option[CMSMS_SHORTNAME . '_blog_post_author']) {
				if ($cmsms_option[CMSMS_SHORTNAME . '_blog_post_cat'] || $cmsms_option[CMSMS_SHORTNAME . '_blog_page_comment'] || $cmsms_option[CMSMS_SHORTNAME . '_blog_post_date']) {
					echo '<span class="cmsms_post_meta_divider">/</span>' . "\n\t\t\t";
				}
			...
			if ($cmsms_option[CMSMS_SHORTNAME . '_blog_page_cat'] && get_the_category()) {
				if ($cmsms_option[CMSMS_SHORTNAME . '_blog_page_comment'] || $cmsms_option[CMSMS_SHORTNAME . '_blog_page_date']) {
					echo '<span class="cmsms_post_meta_divider">/</span>' . "\n\t\t\t";
				}
				echo '<span class="cmsms_category">' . 
				__('in', 'cmsmasters') . 
			...
			if ($cmsms_option[CMSMS_SHORTNAME . '_blog_page_author']) {
				if ($cmsms_option[CMSMS_SHORTNAME . '_blog_page_cat'] || $cmsms_option[CMSMS_SHORTNAME . '_blog_page_comment'] || $cmsms_option[CMSMS_SHORTNAME . '_blog_page_date']) {
					echo '<span class="cmsms_post_meta_divider">/</span>' . "\n\t\t\t";
				}
				echo '<span class="user_name">' . 
					__('by', 'cmsmasters') . ' ';
...
function cmsms_pj_date($page_type = 'page') {
	...
	if ( 
		...
		echo '<span class="cmsms_portfolio_full_date">' . 
				'<abbr class="published" title="' . get_the_date() . '">' . 
					get_the_date() . 
				'</abbr>' . 
			'</span>' . "\n\t\t\t";
...
function cmsms_link($cmsms_id, $type = 'post') {
	...
	if ($type = 'project' && $cmsms_option[CMSMS_SHORTNAME . '_portfolio_project_link'] && $cmsms_project_pj_link_text != '' && $cmsms_project_pj_link_url != '') { 
		echo '<li>' . 
		'<span class="cmsms_details_links"><a href="' . $cmsms_project_pj_link_url . '" title="' . $cmsms_project_pj_link_text . '"' . (($cmsms_project_pj_link_target == 'true') ? ' target="_blank"' : '') . '>' . $cmsms_project_pj_link_text . '</a></span>';
		_e('Project Link', 'cmsmasters') . ':' . 
		'</li>' . "\n\t\t\t"; 
	}
}
...
function cmsms_tl_cat($cmsms_id, $taxonomy, $page_type = 'page') {
	...
	if ($page_type == 'page' && $cmsms_option[CMSMS_SHORTNAME . '_testimonial_page_cat']) {
		...
		if ($cmsms_option[CMSMS_SHORTNAME . '_testimonial_post_date'] || $cmsms_option[CMSMS_SHORTNAME . '_testimonial_post_comment']) {
			echo '<span class="cmsms_post_meta_divider">/</span>' . "\n\t\t\t";
		}
		echo '<span class="cmsms_tl_cat">' . 
			get_the_term_list($cmsms_id, $taxonomy, '', ', ', '') . 
